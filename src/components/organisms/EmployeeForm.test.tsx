...
describe('EmployeeForm', () => {
  test('renders create employee form when there is no employee passed to the component', () => {
    const { getByRole } = render(<EmployeeForm onCreate={() => {}} isSuccess={false} />);
  ...
  test('renders update employee form when employee is passed to the component', () => {
    const { getByRole } = render(<EmployeeForm values={ { id: '1', name: 'employee name example' } } onUpdate={() => {}} />);

    expect((getByRole('textbox') as HTMLInputElement).value).toBe('employee name example')
  })

  test('calls onCreate after filling values and submitting form', async () => {
    const onCreateMock = jest.fn()
    const { getByRole, user } = render(<EmployeeForm onCreate={onCreateMock} isSuccess={false} />);
  ...
  test('calls onUpdate after filling values and submitting form', async () => {
    ...
    const { getByRole, user } = render(<EmployeeForm values={ { id: '1', name: 'employee name example' } } onUpdate={onUpdateMock} />);
  ...
  test('resets form after creating successfully', async () => {
    const { getByRole, user, rerender } = render(<EmployeeForm onCreate={() => {}} isSuccess={false} />);
  ...
  test('show name required error after submitting form with empty name', async () => {
    const { getByText, getByRole, user } = render(<EmployeeForm onCreate={() => {}} isSuccess={false} />);
