...
describe("EmployeeUpdate", () => {
  ...
  test("updates employee after changing", async () => {
    const { findByTestId, getByTestId, user } = renderWithQueryClient(
      <EmployeeUpdate id="1" />
    );
  ...
  test("removes employee when the remove button is clicked", async () => {
    ...
    const { findByTestId, user } = renderWithQueryClient(
      <EmployeeUpdate id="1" />
    );
  ...
  test("shows error when updating fails", async () => {
    ...
    await waitFor(() => {
      expect((window.alert as jest.Mock).mock.calls[0][0].toString()).toBe(
        "Error: Server error"
      );
      expect((console.log as jest.Mock).mock.calls[0][0].toString()).toBe(
        "Error: Server error"
      );
    });
  });
